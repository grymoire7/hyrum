...
depended on by somebody. Some nice examples can be found in [this post on Hyrum's law in Go][he].
...
validate the quality of the variations to ensure they preserve the original
message's meaning while using different wording. The output is formatted in the
...
- Ruby, compatible with 3.1 or greater
- [ruby_llm](https://github.com/crmne/ruby_llm) for AI provider integration
- AI services: OpenAI, Anthropic, Gemini, Ollama, Mistral, DeepSeek, Perplexity,
  OpenRouter, Vertex AI, AWS Bedrock, GPUStack, plus a fake provider for testing
  without a live service
- Output formats: Ruby, JavaScript, Python, Java, text, JSON
...
After checking out the repo, run `bin/setup` to install gem dependencies. From
there, use the tasks below to run the test suite and linter.
...
The validator measures two metrics:

1. **Semantic similarity** (85% or higher required): ensures each variation preserves the meaning of your original message.
   Uses embedding models (OpenAI, Google, etc.) when available, and falls back to a word overlap heuristic if no embedding provider is configured.
2. **Lexical diversity** (30 to 70% required): ensures variations use different words from each other.

The overall quality score is a weighted combination of both metrics.

Semantic similarity works best with embedding models. Configure any embedding
provider (OpenAI, Google, etc.) and Hyrum will use it automatically. If no
embedding provider is configured, validation still works using a word overlap
heuristic.

### `mise run <task>`

- `mise run test`: Run the RSpec suite
- `mise run lint`: StandardRB style check
...
Hyrum is available as open source under the terms of the [MIT License](LICENSE).
